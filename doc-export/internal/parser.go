...
func (p *GoldMarkParser) initMetaData(ctx parser.Context) (*Page, error) {
	...
	shortName, ok := metaData["ShortName"].(string)
	if !ok {
		return nil, fmt.Errorf("%v: %w", "missing or invalid ShortName field in yaml section", ErrMetaDataParsing)
	}
	...
	return &Page{Title: title, Weight: weight, Summary: summary, ShortName: shortName}, nil
}
