...
var queryTests = []QueryTest{
	...
	{"filter by metadata date",
		[]QueryTestPayload{
			{*st.IdFromInt(0), "first-collection", time.Now(), map[string]any{"captured-at": "2026-08-01"}},
			{*st.IdFromInt(1), "first-collection", time.Now(), map[string]any{"captured-at": "2026-08-30"}}},
		"meta.captured-at < \"2026-08-15\"",
		"",
		*st.IdFromInt(0),
		1,
	},
	{"order by metadata date field",
		[]QueryTestPayload{
			{*st.IdFromInt(0), "first-collection", time.Now(), map[string]any{"captured-at": "2026-08-30"}},
			{*st.IdFromInt(1), "first-collection", time.Now(), map[string]any{"captured-at": "2026-08-01"}}},
		"",
		"meta.captured-at",
		*st.IdFromInt(1),
		2,
