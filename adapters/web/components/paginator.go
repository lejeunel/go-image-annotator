	...
	return Nav(Aria("label", "pagination"),
		Ul(
			...
			If(
				...
				MakePaginatorNumberedButton(*parsedBaseURL, lastPage-1, false),
			),
			If(
				lastPage > currentPage,
				MakePaginatorNumberedButton(*parsedBaseURL, lastPage, false),
			),
			If(
			...
			Span(
				Class("font-light"),
				Text(fmt.Sprintf("Showing %v item(s) out of %v", numItems, totalItems)),
			),
		))
}
